...
### Phase 2: 구현 전 추가 검증 — GPT-6 Astra max 설계 리뷰
...
### Phase 3: 구현 — GPT-6 Astra (일반 구현=medium / 테스트·버그 수정·리팩토링=high)
...
### Phase 4: 구현 후 검증 — GPT-6 Astra (QA 설계=max)

| 작업 | 담당 | 사용 스킬/명령 | 시점 |
|---|---|---|---|
| 코드 리뷰·보안 검토 | GPT-6 Astra high | 자체 diff review, `review`/`gstack-review` | 변경 완료 후 |
| QA 시나리오 설계 | GPT-6 Astra max | `gstack-qa` 계획 수립 | 사용자 흐름 변경 시 |
| QA 실행 | GPT-6 Astra high | `qa`, 브라우저 smoke | QA 설계 확정 후 |
| 완료 전 검증 | 작업 수행 effort (medium/high/max) | `superpowers:verification-before-completion` | 완료 보고 전 |
| 문서 업데이트 | GPT-6 Astra medium | 일반 | 사용자·운영 기준 변경 시 |
...
- **설계·기획·QA 설계·디자인·아키텍처·스코프/보안 리스크 판단** → `GPT-6 Astra max`
- **코드 구현·간단한 변경·git·배포** → `GPT-6 Astra medium`
- **테스트·오류 수정·대규모 수정·리뷰·보안 점검/수정 등 복잡한 작업** → `GPT-6 Astra high`
- **Codex 서브에이전트** → 전부 `gpt-6-astra` + `high` (2026-09-16 사용자 지정; 런타임에서도 해당 모델을 명시)
