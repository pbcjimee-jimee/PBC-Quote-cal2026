...
- Main-Materials-to-Option copy is a client-side snapshot transform from the current `materials` state to ordinary `QuoteOptionItem` rows with fresh identities. Existing draft and quote option persistence handles the result; Product / Service lines and DB, RPC, RLS, Server Action, and Jobber write-back paths are unchanged.
...
### 2026-08-13 app performance optimization branch
...
- Inventory initial/search page는 활성 행을 안정적인 `created_at DESC, id DESC` 순서로 1,000행씩 반복 조회해 현재 필터의 전체 결과를 전달한다. 기본 `Current stock` 필터는 `out`만 제외하므로 `in_stock`과 `unknown`은 처음부터 모든 카테고리와 함께 보이고, `Out` 필터는 반출 행 전체를 같은 방식으로 조회한다. 수동 `Load more`는 사용하지 않는다. 이 동작은 2026-08-25 사용자 지시로 기존 50-row 성능 정책을 대체한다. Quote Form draft는 최신 in-memory state를 300ms trailing debounce로 저장하되 `pagehide`/unmount에서 flush한다. Settings Labour Rates만 initial chunk에 남고 네 inactive editor는 선택 시 별도 chunk로 로드한다.
...
승인 경계는 유지한다. DB index, RLS 변경, revision allocation RPC 같은 schema 최적화는 별도 migration review와 production DB 승인 전까지 구현하지 않는다.
