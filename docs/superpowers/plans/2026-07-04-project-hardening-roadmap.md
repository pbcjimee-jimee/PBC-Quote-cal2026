...
> 설계·승인 판단은 Codex 5.6-Sol max, 구현·검증은 Codex 5.6-Sol(일반 구현=medium / 테스트·오류 수정·대규모=high). Production Supabase 변경은 사용자 명시 승인 필요.
...
| 역할 | 담당 모델 | 책임 |
|---|---|---|
| Planner / Designer / Security 판단 | Codex 5.6-Sol max | 접근 모델·백업 정책 결정, UX·보안 리뷰 설계 |
| Developer / DevOps | Codex 5.6-Sol (일반 구현=medium / 테스트·대규모=high) | RLS·RPC·검증·CSP·테스트·운영 문서 구현 |
