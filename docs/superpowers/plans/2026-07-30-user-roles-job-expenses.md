...
1. **역할 기반 접근 제어** — 로그인 사용자를 `admin` / `supervisor` 2개 역할로 분리한다.
   - `admin`: 현재 앱의 모든 기능(견적·설정·progress invoice·inventory·Jobber 연결) + 사용자 관리.
   - `supervisor`: Inventory 접근 + **자기에게 배정된 Jobber job**의 expense·profit 확인만 가능. 견적·가격 설정·progress invoice에는 접근 불가.
...
| 페이지네이션 헬퍼 | `lib/jobber/pagination.ts` | progress invoice에서 사용 중 |
| 전용 Jobber 모듈 분리 패턴 | `lib/jobber/invoice-client.ts` / `invoice-gateway.ts` | 신규 job 모듈도 같은 패턴 적용 |
...
- 신규 Jobber 조회는 전용 모듈(`lib/jobber/job-client.ts`)로 분리. 기존 quote/invoice 모듈·라우트·테스트는 변경 최소화.
...
1. supervisor 로그인 → Jobs·Inventory만 보이고, `/quotes`·`/settings`·`/progress-invoices` 직접 URL 접근 시 자기 홈으로 redirect. Supabase anon 클라이언트로도 admin 테이블 데이터가 0행 (RLS).
2. admin 로그인 → 기존 전 기능 + `/settings/users`에서 사용자 생성·역할 변경·비활성화·Jobber 팀원 연결 가능. Vercel env 변경 없이 사용자 추가 가능.
...
app/(app)/layout.tsx, quotes·settings·progress-invoices 가드          (변경)
