...
| **G2 로컬 데이터 검증** | ✅ 완료 (2026-07-31) — fresh local reset으로 역할 마이그레이션 적용, pgTAP과 실제 local Supabase RLS 및 정적 역할·supervisor 차단 검증 통과. strict separation 후 최종 수치는 Task 6에서 다시 기록 | `tests/rls.test.ts`, `tests/rls-local-integration.test.ts`, `tests/job-snapshot-migration.test.ts`, `tests/supervisor-route-security.test.ts`, 로컬 reset/test 로그 | 프로덕션 DB로 검증 대체 금지 |
