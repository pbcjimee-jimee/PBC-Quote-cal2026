...
> 모든 작업은 **GPT-6 Astra**로 진행하고 역할에 따라 effort를 나눈다.
...
| 영역 | 담당 모델 | 기준 |
|---|---|---|
| 설계 (Design)·아키텍처 | **GPT-6 Astra max** | 사용자 요구, `docs/ARCHITECTURE.md`, `docs/DB-SCHEMA.md`, `docs/SECURITY.md` |
| 아이디어·브레인스토밍 | **GPT-6 Astra max** | 사용자 요구, superpowers/gstack 스킬 |
| UI/UX 디자인 | **GPT-6 Astra max** | `docs/UI-DESIGN-SYSTEM.md`, `docs/UI-DESIGN.md`, `docs/UI-UX-REVIEW.md` |
| QA 시나리오·테스트 전략 | **GPT-6 Astra max** | `docs/DECISIONS.md` 테스트 정책 |
| 기능 구현·간단한 변경 | **GPT-6 Astra medium** | 기존 코드 패턴, 설계 문서 |
| 테스트 작성 | **GPT-6 Astra high** | test plan, 설계 문서 |
| 버그·오류 수정, 대규모 수정 | **GPT-6 Astra high** | 재현 → 원인 확인 → 최소 수정 → 검증 |
| 코드 리뷰·보안 | **GPT-6 Astra high** | diff 기반 리뷰, 필요 시 `gstack-review`·`review` |
| git·배포 실행 | **GPT-6 Astra medium** | `docs/DEPLOY.md`, `docs/CLI-ACCESS.md`, 사용자 승인 규칙 |
...
- **무엇을·왜**(설계)는 GPT-6 Astra max가 사용자와 협의해 결정하고 `docs/`에 남긴다.
- **어떻게**(구현·검증)는 GPT-6 Astra medium/high가 그 문서를 입력으로 수행한다.
...
1. 사용자 요청
   ↓
2. [GPT-6 Astra max] 문제 정의와 요구사항 확인
   - 목적, 데이터 출처, 권한, 파일 출력, 성공 기준 확인
   - 산출물: 설계 요약 또는 design doc (docs/superpowers/specs/)
   ↓
3. [GPT-6 Astra max] 아키텍처·테스트·엣지 케이스 정리
   - DB/RLS/API/UI/테스트 범위 잠금
   - 산출물: 구현 계획 (docs/superpowers/plans/)
   ↓
4. [GPT-6 Astra medium/high] 구현
   - medium: DB 마이그레이션 → Server Actions/Route Handlers → UI
   - high: 테스트 작성·버그 수정·대규모 리팩토링
   ↓
5. [GPT-6 Astra high] 테스트·검증
   - typecheck, lint, test, 필요 시 browser QA
   ↓
6. [GPT-6 Astra medium] 완료 보고 → PROGRESS.md 반영
   - 변경 파일, 테스트, 남은 리스크, 다음 단계
```

간단한 변경은 설계 단계를 짧은 요약으로 압축할 수 있으나, DB/RLS/보안/공식이 걸리면 반드시 GPT-6 Astra max 설계 문서를 먼저 확정한 뒤 GPT-6 Astra medium/high가 구현한다.
테스트 작성·오류 수정·대규모 수정이 주 작업이면 4~6단계를 **GPT-6 Astra high**가 수행한다.
