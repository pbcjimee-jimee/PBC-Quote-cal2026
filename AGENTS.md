...
| 역할군 | 담당 모델 | 하는 일 |
|---|---|---|
| **설계·기획** | **GPT-6 Astra max** | 계획·아키텍처 설계, 아이디어·브레인스토밍, QA 시나리오 설계, UI/UX 디자인, plan/design 리뷰, 스코프·보안 리스크 판단 |
| **구현 (간단·일반)** | **GPT-6 Astra medium** | 코딩(마이그레이션·Server Action·Route·UI), 간단한 변경, 단순 문구 수정·기계적 반복, git 작업, 배포 실행 |
| **복잡·장시간 작업** | **GPT-6 Astra high** | 테스트 작성, 버그·오류 수정, 대규모 수정·리팩토링, 코드 리뷰·보안 점검·수정, 오래 걸리는 작업 |

원칙:
- **무엇을·왜 만들지(설계)** 는 **GPT-6 Astra max**가 사용자와 협의해 결정하고 문서로 남긴다.
- **어떻게 코드로 구현·검증·반영할지(실행)** 는 GPT-6 Astra가 문서·코드 패턴에 따라 수행한다. 코드 구현·간단한 변경은 **medium**, 테스트·오류 수정·대규모 수정 등 복잡한 작업은 **high**를 쓴다.
- **Codex 서브에이전트 스폰 시** 서브에이전트 모델은 전부 **`gpt-6-astra` + high**로 실행한다(2026-09-16 사용자 지정; 런타임에서도 해당 모델을 명시).
...
| 작업 | 담당 모델 | 필독 파일 |
|---|---|---|
| 신규 기능 설계 | GPT-6 Astra max | `PROGRESS.md` → `docs/DECISIONS.md` → `docs/ARCHITECTURE.md` → `docs/SECURITY.md` |
| DB 마이그레이션 | GPT-6 Astra medium | `docs/DB-SCHEMA.md` → `docs/SECURITY.md` |
| 계산 로직 | GPT-6 Astra medium | `docs/CALCULATION.md` → `docs/CALCULATION-API.md` → `docs/CODING-STYLE.md` |
| Server Actions | GPT-6 Astra medium | `docs/ARCHITECTURE.md` → `docs/DB-SCHEMA.md` → `docs/CODING-STYLE.md` |
| UI/UX 디자인 설계 | GPT-6 Astra max | `docs/UI-DESIGN-SYSTEM.md` → `docs/UI-DESIGN.md` → `docs/UI-UX-REVIEW.md` |
| UI 컴포넌트 구현 | GPT-6 Astra medium | `docs/UI-DESIGN-SYSTEM.md` → 페이지별 명세 → `docs/CODING-STYLE.md` |
| 테스트 작성 | GPT-6 Astra high | `docs/CALCULATION.md` → `docs/CALCULATION-API.md` → `PROGRESS.md` |
| 버그·오류 수정, 대규모 수정 | GPT-6 Astra high | 재현 경로 → 관련 명세 → `docs/CODING-STYLE.md` |
| 코드 리뷰·보안 | GPT-6 Astra high | `docs/DECISIONS.md` → `docs/CODING-STYLE.md` → `docs/SECURITY.md` |
| 배포 | GPT-6 Astra medium | `docs/DEPLOY.md` → `docs/CLI-ACCESS.md` → `docs/SECURITY.md` |
...
**Model:** GPT-6 Astra max (설계) / GPT-6 Astra medium (구현·간단한 변경) / GPT-6 Astra high (테스트·오류 수정·대규모)
...
| 설계와 구현 담당이 갈릴 때 | GPT-6 Astra max가 설계를 문서로 확정 → GPT-6 Astra medium/high가 그 문서 기준으로 구현 |
