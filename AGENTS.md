...
| 역할군 | 담당 모델 | 하는 일 |
|---|---|---|
| **설계·기획** | **Codex 5.6-Sol max** | 계획·아키텍처 설계, 아이디어·브레인스토밍, QA 시나리오 설계, UI/UX 디자인, plan/design 리뷰, 스코프·보안 리스크 판단 |
| **구현 (간단·일반)** | **Codex 5.6-Sol medium** | 코딩(마이그레이션·Server Action·Route·UI), 간단한 변경, 단순 문구 수정·기계적 반복, git 작업, 배포 실행 |
| **복잡·장시간 작업** | **Codex 5.6-Sol high** | 테스트 작성, 버그·오류 수정, 대규모 수정·리팩토링, 코드 리뷰·보안 점검·수정, 오래 걸리는 작업 |

원칙:
- **무엇을·왜 만들지(설계)** 는 **Codex 5.6-Sol max**가 사용자와 협의해 결정하고 문서로 남긴다.
- **어떻게 코드로 구현·검증·반영할지(실행)** 는 Codex 5.6-Sol이 문서·코드 패턴에 따라 수행한다. 코드 구현·간단한 변경은 **medium**, 테스트·오류 수정·대규모 수정 등 복잡한 작업은 **high**를 쓴다.
...
| 작업 | 담당 모델 | 필독 파일 |
|---|---|---|
| 신규 기능 설계 | Codex 5.6-Sol max | `PROGRESS.md` → `docs/DECISIONS.md` → `docs/ARCHITECTURE.md` → `docs/SECURITY.md` |
| DB 마이그레이션 | Codex 5.6-Sol medium | `docs/DB-SCHEMA.md` → `docs/SECURITY.md` |
| 계산 로직 | Codex 5.6-Sol medium | `docs/CALCULATION.md` → `docs/CALCULATION-API.md` → `docs/CODING-STYLE.md` |
| Server Actions | Codex 5.6-Sol medium | `docs/ARCHITECTURE.md` → `docs/DB-SCHEMA.md` → `docs/CODING-STYLE.md` |
| UI/UX 디자인 설계 | Codex 5.6-Sol max | `docs/UI-DESIGN-SYSTEM.md` → `docs/UI-DESIGN.md` → `docs/UI-UX-REVIEW.md` |
| UI 컴포넌트 구현 | Codex 5.6-Sol medium | `docs/UI-DESIGN-SYSTEM.md` → 페이지별 명세 → `docs/CODING-STYLE.md` |
| 테스트 작성 | Codex 5.6-Sol high | `docs/CALCULATION.md` → `docs/CALCULATION-API.md` → `PROGRESS.md` |
| 버그·오류 수정, 대규모 수정 | Codex 5.6-Sol high | 재현 경로 → 관련 명세 → `docs/CODING-STYLE.md` |
| 코드 리뷰·보안 | Codex 5.6-Sol high | `docs/DECISIONS.md` → `docs/CODING-STYLE.md` → `docs/SECURITY.md` |
| 배포 | Codex 5.6-Sol medium | `docs/DEPLOY.md` → `docs/CLI-ACCESS.md` → `docs/SECURITY.md` |
...
**Model:** Codex 5.6-Sol max (설계) / Codex 5.6-Sol medium (구현·간단한 변경) / Codex 5.6-Sol high (테스트·오류 수정·대규모)
...
| 설계와 구현 담당이 갈릴 때 | Codex 5.6-Sol max가 설계를 문서로 확정 → Codex 5.6-Sol medium/high가 그 문서 기준으로 구현 |
