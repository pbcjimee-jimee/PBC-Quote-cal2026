...
- **역할 분업:** 모든 작업은 **Codex 5.6-Sol**이 담당한다 — 설계·기획·QA 설계·디자인·아키텍처·스코프/보안 리스크 판단은 **max**, 코드 구현·간단한 변경은 **medium**, 테스트·오류 수정·대규모 수정·리뷰·보안 점검/수정은 **high**.
- **흐름:** 새 기능은 [Sol max] 요구사항 확인 → 설계 doc → 구현 계획 → [Sol medium/high] 구현 → [Sol high] 테스트·검증 순서로 진행한다.
