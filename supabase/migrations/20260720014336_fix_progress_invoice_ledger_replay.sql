CREATE OR REPLACE FUNCTION public.reject_progress_adjustment(payload JSONB)
RETURNS TABLE (
...
CREATE OR REPLACE FUNCTION public.create_manual_progress_payment(payload JSONB)
RETURNS TABLE (
...
BEGIN
  ...
  IF prior IS NOT NULL THEN
    RETURN QUERY SELECT (prior->>'id')::UUID,(prior->>'series_id')::UUID,(prior->>'series_version')::INT,
      (prior->>'version')::INT,(prior->>'revision_id')::UUID,false,NULL::JSONB;
    RETURN;
  END IF;
  IF series_row.status='void' THEN RAISE EXCEPTION 'PROGRESS_SERIES_VOID' USING ERRCODE='P0001'; END IF;
  IF series_row.version <> (payload->>'expected_series_version')::INT THEN
    RETURN QUERY SELECT NULL::UUID,series_row.id,series_row.version,NULL::INT,NULL::UUID,true,
      jsonb_build_object('series_id',series_row.id,'series_version',series_row.version);
    RETURN;
  END IF;
...
CREATE OR REPLACE FUNCTION public.replace_manual_progress_payment(payload JSONB)
...
BEGIN
  ...
  IF series_row.status='void' THEN RAISE EXCEPTION 'PROGRESS_SERIES_VOID' USING ERRCODE='P0001'; END IF;
  IF payment_row.version<>(payload->>'expected_version')::INT THEN
    RETURN QUERY SELECT payment_row.id,series_row.id,series_row.version,payment_row.version,current_revision.id,true,
      public.progress_payment_safe_dto(payment_row,current_revision);
    RETURN;
  END IF;
  ...
  PERFORM public.progress_recalculate_series_read_model_as(series_row.id,actor);
  result_refs:=jsonb_build_object('id',payment_row.id,'series_id',series_row.id,'series_version',series_row.version,
    'version',payment_row.version,'revision_id',next_revision.id);
  PERFORM public.progress_append_event(series_row.id,NULL,'manual_payment_replaced','user',NULL,NULL,
    jsonb_build_object('payment_id',payment_row.id),'replace_manual_progress_payment',key,fingerprint,result_refs);
...
CREATE OR REPLACE FUNCTION public.void_manual_progress_payment(payload JSONB)
RETURNS TABLE (id UUID,series_id UUID,series_version INT,version INT,revision_id UUID,conflict BOOLEAN,current JSONB)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  INSERT INTO public.progress_payment_revisions(payment_id,revision_number,received_date,observed_amount,effective_receipt_amount,
    payment_method,reference,sync_state,status,predecessor_revision_id,created_by,reason)
  VALUES(payment_row.id,current_revision.revision_number+1,current_revision.received_date,current_revision.observed_amount,0,
    current_revision.payment_method,current_revision.reference,'manual','void',current_revision.id,actor,
    public.progress_ledger_validate_reason(payload->>'reason')) RETURNING * INTO next_revision;
  UPDATE public.progress_payments AS payment SET current_revision_id=next_revision.id,version=payment.version+1,updated_by=actor
  WHERE payment.id=payment_row.id RETURNING payment.* INTO payment_row;
  UPDATE public.progress_invoice_series AS series SET version=series.version+1,updated_by=actor
  ...
  result_refs:=jsonb_build_object('id',payment_row.id,'series_id',series_row.id,'series_version',series_row.version,
    'version',payment_row.version,'revision_id',next_revision.id);
  PERFORM public.progress_append_event(series_row.id,NULL,'manual_payment_voided','user',NULL,NULL,
    jsonb_build_object('payment_id',payment_row.id),'void_manual_progress_payment',key,fingerprint,result_refs);
  RETURN QUERY SELECT payment_row.id,series_row.id,series_row.version,payment_row.version,next_revision.id,false,NULL::JSONB;
END;
$$;

CREATE OR REPLACE FUNCTION public.reconcile_progress_payment(payload JSONB)
RETURNS TABLE (id UUID,series_id UUID,series_version INT,version INT,revision_id UUID,conflict BOOLEAN,current JSONB)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
DECLARE
  actor UUID:=public.progress_require_actor();
  series_row public.progress_invoice_series%ROWTYPE;
  jobber public.progress_payments%ROWTYPE;
  manual public.progress_payments%ROWTYPE;
  jobber_revision public.progress_payment_revisions%ROWTYPE;
  manual_revision public.progress_payment_revisions%ROWTYPE;
  next_manual public.progress_payment_revisions%ROWTYPE;
  key UUID;
  fingerprint TEXT;
  prior JSONB;
  result_refs JSONB;
...
BEGIN
  ...
  IF jobber.series_id<>manual.series_id THEN
  ...
  IF prior IS NOT NULL THEN
    ...
    RETURN;
  END IF;
  SELECT revision.* INTO STRICT jobber_revision FROM public.progress_payment_revisions AS revision
  WHERE revision.id=jobber.current_revision_id AND revision.payment_id=jobber.id;
  SELECT revision.* INTO STRICT manual_revision FROM public.progress_payment_revisions AS revision
  WHERE revision.id=manual.current_revision_id AND revision.payment_id=manual.id;
  IF series_row.status='void' THEN RAISE EXCEPTION 'PROGRESS_SERIES_VOID' USING ERRCODE='P0001'; END IF;
  IF jobber.version<>(payload->>'jobber_expected_version')::INT THEN
    RETURN QUERY SELECT jobber.id,series_row.id,series_row.version,jobber.version,manual_revision.id,true,
      public.progress_payment_safe_dto(jobber,jobber_revision);
    RETURN;
  END IF;
  IF manual.version<>(payload->>'manual_expected_version')::INT THEN
    RETURN QUERY SELECT manual.id,series_row.id,series_row.version,manual.version,manual_revision.id,true,
      public.progress_payment_safe_dto(manual,manual_revision);
    RETURN;
  END IF;
  ...
  INSERT INTO public.progress_payment_revisions(payment_id,revision_number,received_date,observed_amount,effective_receipt_amount,
    payment_method,reference,sync_state,status,predecessor_revision_id,created_by,reason)
  VALUES(manual.id,manual_revision.revision_number+1,manual_revision.received_date,manual_revision.observed_amount,0,
    manual_revision.payment_method,manual_revision.reference,'manual','superseded',manual_revision.id,actor,
    public.progress_ledger_validate_reason(payload->>'reason')) RETURNING * INTO next_manual;
  UPDATE public.progress_payments AS payment SET current_revision_id=next_manual.id,version=payment.version+1,updated_by=actor
  WHERE payment.id=manual.id RETURNING payment.* INTO manual;
  UPDATE public.progress_payments AS payment SET matched_manual_payment_id=manual.id,version=payment.version+1,updated_by=actor
  WHERE payment.id=jobber.id RETURNING payment.* INTO jobber;
  UPDATE public.progress_invoice_series AS series SET version=series.version+1,updated_by=actor
  WHERE series.id=series_row.id RETURNING series.* INTO series_row;
  PERFORM public.progress_recalculate_series_read_model_as(series_row.id,actor);
  result_refs:=jsonb_build_object('id',jobber.id,'series_id',series_row.id,'series_version',series_row.version,
    'version',jobber.version,'revision_id',next_manual.id);
  PERFORM public.progress_append_event(series_row.id,NULL,'payment_reconciled','user',NULL,NULL,
    jsonb_build_object('payment_id',jobber.id),'reconcile_progress_payment',key,fingerprint,result_refs);
  RETURN QUERY SELECT jobber.id,series_row.id,series_row.version,jobber.version,next_manual.id,false,NULL::JSONB;
END;
$$;

CREATE OR REPLACE FUNCTION public.undo_progress_payment_reconciliation(payload JSONB)
RETURNS TABLE (id UUID,series_id UUID,series_version INT,version INT,revision_id UUID,conflict BOOLEAN,current JSONB)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  ...
  INSERT INTO public.progress_payment_revisions(payment_id,revision_number,received_date,observed_amount,effective_receipt_amount,
    payment_method,reference,sync_state,status,predecessor_revision_id,created_by,reason)
  VALUES(manual.id,manual_revision.revision_number+1,manual_revision.received_date,manual_revision.observed_amount,
    manual_revision.observed_amount,manual_revision.payment_method,manual_revision.reference,'manual','active',manual_revision.id,
    actor,public.progress_ledger_validate_reason(payload->>'reason')) RETURNING * INTO next_manual;
  UPDATE public.progress_payments AS payment SET current_revision_id=next_manual.id,version=payment.version+1,updated_by=actor
  WHERE payment.id=manual.id RETURNING payment.* INTO manual;
  UPDATE public.progress_payments AS payment SET matched_manual_payment_id=NULL,version=payment.version+1,updated_by=actor
  WHERE payment.id=jobber.id RETURNING payment.* INTO jobber;
  UPDATE public.progress_invoice_series AS series SET version=series.version+1,updated_by=actor
  WHERE series.id=series_row.id RETURNING series.* INTO series_row;
  PERFORM public.progress_recalculate_series_read_model_as(series_row.id,actor);
  result_refs:=jsonb_build_object('id',jobber.id,'series_id',series_row.id,'series_version',series_row.version,
    'version',jobber.version,'revision_id',next_manual.id);
  PERFORM public.progress_append_event(series_row.id,NULL,'payment_reconciliation_undone','user',NULL,NULL,
    jsonb_build_object('payment_id',jobber.id),'undo_progress_payment_reconciliation',key,fingerprint,result_refs);
  RETURN QUERY SELECT jobber.id,series_row.id,series_row.version,jobber.version,next_manual.id,false,NULL::JSONB;
END;
$$;
