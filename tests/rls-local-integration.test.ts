...
describeLocal('Supabase local RLS CRUD integration', () => {
  ...
  it('allows authenticated CRUD on RLS-protected app tables', async () => {
    ...
    expect((await authed.from('quotes').delete().eq('id', quoteData.id)).error?.code).toBe('42501')
    expect((await admin.from('quotes').delete().eq('id', quoteData.id)).error?.code).toBe('42501')
    expectNoError(await authed.rpc('soft_delete_quote', { target_quote_id: quoteData.id, expected_version: 1 }))
    expect(expectNoError(await authed.from('quotes').select('id').eq('id', quoteData.id).is('deleted_at', null)).data).toEqual([])
    expect(expectNoError(await supervisor.from('quotes').select('id').eq('id', quoteData.id).not('deleted_at', 'is', null)).data).toEqual([])
    expect((await supervisor.rpc('restore_quote', { target_quote_id: quoteData.id, expected_version: 2 })).error?.code).toBe('42501')
    expect((await anon.rpc('restore_quote', { target_quote_id: quoteData.id, expected_version: 2 })).error?.code).toBe('42501')
    const retained = expectNoError(await authed.from('quote_items').select('id').eq('id', itemData.id).single())
    expect(retained.data?.id).toBe(itemData.id)
    expectNoError(await authed.rpc('restore_quote', { target_quote_id: quoteData.id, expected_version: 2 }))
    expectNoError(await authed.from('quote_items').delete().eq('id', itemData.id))
    expectNoError(await authed.rpc('soft_delete_quote', { target_quote_id: quoteData.id, expected_version: 3 }))
