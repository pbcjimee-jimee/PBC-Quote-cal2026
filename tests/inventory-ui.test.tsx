...
describe('inventory UI', () => {
  ...
  it('groups items by category and makes out rows visually distinct', () => {
    ...
    // 카테고리 헤더 시각 스타일은 components.css의 pbc-* 클래스가 단일 소스다 — inline style 중복 금지
    expect(markup).not.toContain('background:linear-gradient(135deg, var(--primary-soft), #fff)')
    expect(markup).not.toContain('border-left:5px solid var(--primary)')
