...
| 2026-05-19 | Jobber quote fetch throttling hardened. Full quote fetch/search now falls back to lightweight quote queries without custom fields or client tags when Jobber rejects or repeatedly throttles the full query, while still loading up to 100 Product / Service line items. Added regression tests for encoded quote fetch, quote-number search, and exhausted temporary throttle fallback. | Codex |
