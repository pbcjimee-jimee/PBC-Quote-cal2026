...
- **UX 잔여**: `docs/UI-UX-REVIEW.md`(2026-07-28 전면 재리뷰) 기준 R1~R3은 2026-07-28 반영 완료. 남은 것은 R4(인터랙션 함정·디자인 시스템 위반 잔여·반응형/터치 잔여)와 R5(P2/P3 폴리시). UX 부채는 `docs/BACKLOG.md` P6에 등록됨(2026-07-28 사용자 승인).
...
| 날짜 | 작업 | 담당 |
|---|---|---|
| 2026-07-28 | UI/UX 리뷰 R3 구현. 대비 토큰 재배치(`--muted` #556070·`--muted-2` #6b7789, 상태색 텍스트 *-text 토큰 전환 — progress badge·iconbtn--danger·areachip 포함), Imported muted 배지·Import soft 버튼, error/not-found/global-error 바운더리 신설, quotes·settings loading 실 레이아웃 정합 + `/quotes/[id]`·`/quotes/[id]/edit` loading 신설(`pbc-skeleton`), 검색 useTransition pending 스피너, Save & Sync `?synced=1` 성공 배너(jobberSyncStatus 게이팅 + 1회성 URL 소비), Settings 로드 실패 시 Save 비활성(`pricingLoadFailed`). diff 적대적 리뷰 6건 전건 반영. 검증: typecheck·lint·Vitest 799 tests·production build 통과 + 로컬 :3000 브라우저 검증(토큰 적용·모바일 44px/16px·콘솔 에러 0건). | Claude |
| 2026-07-28 | UI/UX 리뷰 R2 구현 + UX 부채 BACKLOG 등록(사용자 승인). `Alert` 프리미티브 신설(tone별 role/aria-live 자동, live=false 지원) 후 quote-form·paint-search·customer-panel·inventory·settings-form·서버 페이지 3곳의 동적 피드백 교체. DecimalInput useId 폴백, F1~F5 라디오 aria-label + chip 포커스 링, Jobber Refresh aria-busy·액션 반환 status 기준 3분기 성공 피드백(8초 자동 해제). 금액 위계: FinalSummary 중복 subtotal 행·상세 Summary 중복 소계 그리드 제거, Settings 로드 실패 문구에 기본값 덮어쓰기 경고. diff 적대적 리뷰(2 에이전트) 발견 7건 전건 반영(stale prop 문구 오류, 정적 알림 live 제거, form role 오용 등). 검증: typecheck·lint·Vitest 84 files/799 tests·production build 통과. | Claude |
