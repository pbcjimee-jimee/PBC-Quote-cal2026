...
| 날짜 | 작업 | 담당 |
|---|---|---|
| 2026-08-20 | Overview 견적 검색을 고객명·주소뿐 아니라 Jobber ID와 `jobber_snapshot.quoteNumber`까지 확장했다. `#3535`·`# 3535` 입력을 지원하고, PostgREST 인용 및 `ILIKE` 메타문자 escaping으로 기존 구두점 검색과 운영/dev 검색 동작을 보존했다. TDD focused 2 files/75 tests와 최종 전체 verify를 통과했다: Vitest 93 files/813 tests(1 file/9 tests skip), coverage 84.77/71.51/94.39/90.16%, strict TypeScript, ESLint, production build 18 routes, production audit 0 vulnerabilities. 독립 재리뷰의 Critical/Important finding은 0건이다. DB migration과 새 의존성 추가는 수행하지 않았다. | Codex 5.6-Sol high |
