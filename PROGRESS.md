...
| 날짜 | 작업 | 담당 |
|---|---|---|
| 2026-07-17 | 성능 Phase 3 구현. ① 견적 폼 코드 스플리팅: `JobberProductServiceEditor`·`JobberOptionImport`를 `components/quote-form/lazy-panels.tsx`의 `next/dynamic`(ssr:false)으로 분리 — 실측상 두 패널이 라우트 초기 청크에서 별도 async 청크로 이동(하이드레이션 임계 경로 제외), 옵션 임포트는 후보 존재 시에만 렌더해 일반 경로에서 청크 미로드, settings template 탭도 동일 적용(/quotes/new page-own 56.7→55.0kB, /settings 32.7→31.2kB gzip). ② 타이핑 렉 제거: 매 키 입력마다 돌던 전체 draft `JSON.stringify` 2회(dirty 비교 + localStorage 저장)를 shallow 참조 비교(`isDraftEquivalent`) + 400ms debounce 저장으로 교체(beforeunload·나가기 다이얼로그는 동기 flush 유지). 리뷰에서 debounce 타이머가 "Leave without draft"로 버린 draft를 부활시키는 major 회귀를 확정 → `writeDraftToStorage`에 isNavigatingRef 가드 추가로 수정(저장 성공 경로도 동일 커버). 회귀 자동 테스트는 테스트 DOM(history/removeItem 부재) 확장이 선행돼야 해 별도 작업으로 분리. ③ cacheComponents/PPR은 전역 렌더 변경 리스크 대비 이득 marginal로 스킵(감사 결론 유지). 검증: typecheck/lint/Vitest 84 files·803 tests/build 통과, 2관점 리뷰+적대적 검증(major 1건 발견·수정). | Claude Fable 5 |
